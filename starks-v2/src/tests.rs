...
impl ConstraintProvider for FibonacciConstraintProvider {
    fn get_constraints_evaluations<FieldType: Field + Clone>(
        ...
        let t_coeffs_shifted_w = poly_shift_coeffs(trace_coeffs, w, p);
        let t_coeffs_shifted_w2 = poly_shift_coeffs(trace_coeffs, mod_pow(w, 2, p), p);

        let n_coeffs_term1 = poly_sub_coeffs(&t_coeffs_shifted_w2, &t_coeffs_shifted_w, p);
        let n_coeffs = poly_sub_coeffs(&n_coeffs_term1, trace_coeffs, p);
...
fn test_fibonacci_stark() {
    ...
    let proof = stark_instance.create_proof_1d(
        fib_trace.clone(),
        blowup_factor,
        fri_num_rounds,
        num_queries,
    ...
    let result_bad_trace = std::panic::catch_unwind(|| {
        stark_instance.create_proof_1d(
            bad_fib_trace.clone(),
            blowup_factor,
            fri_num_rounds,
            num_queries,
